...
namespace sf {
    ...
    class safe_obj {
        protected:
            ...
            T * get_obj_ptr() const { return (T*)(&obj); }
    ...
    class safe_hide_obj : protected safe_obj<T, mutex_t, x_lock_t, s_lock_t> {
        public:
            ...
            explicit operator T() const { return (safe_obj<T, mutex_t, x_lock_t, s_lock_t>)(*this); };
	...
	template<size_t lock_count, typename duration = std::chrono::nanoseconds,
		...
		public:
            ...
            lock_timed_any(lock_timed_any&& other) : locks_ptr_vec(other.locks_ptr_vec) { }
    ...
    struct xlocked_safe_ptr {
        ...
        xlocked_safe_ptr(T const& p) : ref_safe(*(T*)(&p)), xlock(*(ref_safe.get_mtx_ptr())) {}// ++xp;}
    ...
    struct slocked_safe_ptr {
        ...
        slocked_safe_ptr(T const& p) : ref_safe(*(T*)(&p)), slock(*(ref_safe.get_mtx_ptr())) { }//++sp;}
